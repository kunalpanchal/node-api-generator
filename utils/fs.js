...
module.exports = {
    writeFile: (fileName, content, cb) => {
        if (content)
            fs.writeFile(fileName, content, (err) => logStatus(err, fileName));
        else
            fs.open(fileName, 'w', function (err) { logStatus(err, fileName); });
        cb && cb();
    },
    mkdir: (dirName, cb) => {
        dirName && fs.mkdir(dirName, (err) => logStatus(err, dirName + '/'));
        cb && cb(dirName);
    },
    mkdirSync: (dirName) => {
